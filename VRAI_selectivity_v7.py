...
def main1(filename1, filename2, filename3, filename4, filename5, filename6, filename7, filename8, intermediate_option, weight_option, TST_option=True, temperature = 298.0, with_spe=False,
          energy_cut_off=5):

    code_name = 'VRAI-selectivity_v5_'

    logging.basicConfig(filename=code_name+filename1.split('/')[-1][:-4]+'.log', level=logging.DEBUG, format='%(message)s', filemode='w')
    ...
    rdkitmolTS2 = Chem.MolFromMolFile(filename2, removeHs=False, strictParsing=False, sanitize=False)
    if not rdkitmolTS2:
    ...
    if intermediate_option == True and filename7 != None and filename8 != None:
        

        
        TS1_G = ReadFreeEnergy(filename5, spe = with_spe)
        INT_G = ReadFreeEnergy(filename6, spe = with_spe)
        TS2A_G = ReadFreeEnergy(filename7, spe = with_spe)
        TS2B_G = ReadFreeEnergy(filename8, spe = with_spe)
        ...
        if TS_energydiff < energy_cut_off:
            logging.info('The TS2 free energy is higher than '+str(-1*energy_cut_off)+' kJ/mol from TS1.')
            logging.info('')
            print('Warning: The TS2 free energy is higher than '+str(-1*energy_cut_off)+' kJ/mol from TS1; TST_option = True is perferred')
            print('The higher-energy TS2 is '+str(-1*round(TS_energydiff,2))+' kJ/mol (free energy) from TS1.')
            TST_valid = True
            
            
        else:
            logging.info('The TS2 free energy is lower than '+str(-1*energy_cut_off)+' kJ/mol from TS1.')
            logging.info('')
            print('The higher-energy TS2 is '+str(-1*round(TS_energydiff,2))+' kJ/mol (free energy) from TS1.')
            TST_valid = False
    else:
    ...
    if TST_valid == True:
        ...
        if TS2A_G < TS2B_G:
            ...
            major_per = round(major_perc,1)
            minor_per = round(minor_perc,1)
        ...
        if TS2B_G < TS2A_G:
            ...
            print('')
            print('TST Major product percentage is '+str(round(major_perc*100,1)))
            print('TST Minor product percentage is '+str(round(minor_perc*100,1)))
            print('')
            print('****************************')
            ...
            major_per = round(major_perc*100,1)
            minor_per = round(minor_perc*100,1)
    ...
    if TST_valid == False:
        ...
        if np.sign(mu1_) != np.sign(mu2_) and lambda1_ > 0 and lambda2_ > 0:
            ...
            major_per = round(major_ratio*100, 1)
            minor_per = round(minor_ratio*100, 1)
    ...
    filename1_format=filename1.split('/')[-1][:-4]
    filename2_format=filename2.split('/')[-1][:-4]
    major_format=major.split('/')[-1][:-4]
    minor_format=minor.split('/')[-1][:-4]
    
    result_df=pd.DataFrame({'TS1_name':[filename1_format],'int_name':[filename2_format],'major':[major_format],
                            'minor':[minor_format],'major_per':[major_per], 'minor_per':[minor_per],
                            'TST_valid':[TST_valid]})
...
def main2(filename1, filename2, with_spe=False):
    ...
    TS2A_G = ReadFreeEnergy(filename1, spe = with_spe)
    TS2B_G = ReadFreeEnergy(filename2,  spe = with_spe)
...
def ReadFreeEnergy(GOutpFile, spe=False):
    '''
    ReadEnergy: returns the Gibbs free energy of the Gaussian frequency output file
    ...
    if spe == False:
        ...
        for line in GOutp:
            ...
            if 'Sum of electronic and thermal Free Energies' in line:
                linelist = line.split()
                Free_Energy = float(linelist[-1])

    elif spe == True:
        cor_gibbs_free_energy = thermal_correction_energy(GOutpFile)
        spe_energy = getSPE(GOutpFile[:-4]+'_spe.out')
        Free_Energy = cor_gibbs_free_energy+spe_energy

    # convert form hartree to kJ mol-1
    Free_Energy = Free_Energy * 2625.5002
...
def ReadEigenVec(GOutpFile):
    '''
    ReadEigenVec: returns eigenvectors in ascending order (as print(d in Gaussian output file) of frequency.
...
def CalcXs(FClist, Temp, FrequencyList):
    ...
    for item in FClist:
        ...
        ## aviod division by zero error 
        v = item
        if item == 0:
            v = 0.00004

        FClist_J += [v*(1/15.569141)*2625500.2*(1/BohrtoAngstrom)**2]  # units J mol^-1 A^-2
...
if __name__ == '__main__':
    ...
    parser.add_argument('GausFile1', help="Without -S activation: Gaussian mol output file No.1: TS1, With -S activation: Gaussian output file No.1: TS2A")
    parser.add_argument('GausFile2', help="Without -S activation: Gaussian mol output file No.2: TS2, With -S activation: Gaussian output file No.2: TS2B")
    parser.add_argument('GausFile3', nargs='?', help="Without -S activation: Gaussian mol output file No.3: P1")
    parser.add_argument('GausFile4', nargs='?', help="Without -S activation: Gaussian mol output file No.4: P2")
    parser.add_argument('GausFile5', nargs='?', help="Without -S activation: Gaussian frequency output file for TS1")
    parser.add_argument('GausFile6', nargs='?', help="Without -S activation: Gaussian frequency output file for INT")
    parser.add_argument('GausFile7_option', nargs='?', help='Gaussian frequency output file for TS2A')
    parser.add_argument('GausFile8_option', nargs='?', help='Gaussian frequency output file for TS2B')
    parser.add_argument("-I", "--intermediate_option", help='Activates selectivity prediction method for reactions with a dynamic intermediate', action='store_true')
    ...
    parser.add_argument("-S", "--noTS1_option", help='Performs TST analysis with just the starting material and the two transition states', action='store_true')
    ...
    if args.noTS1_option == True and args.intermediate_option == True:
        print('')
        print('ERROR')
        print('Intermediate activation and the starting material activation are both on. Please only have one activation and provide the appropriate files for the chosen activation')
        print('')
        quit()

    if args.noTS1_option == False:
        if args.GausFile3 == None or args.GausFile4 == None or args.GausFile5 == None or args.GausFile6 == None:
            print('')
            print('ERROR')
            print('Please provide all the appropriate .mol and Gaussian .out files when the starting material activation is off')
            print('')
            quit()
    else:
        if args.GausFile3 != None:
            print('')
            print('ERROR')
            print('When the starting material activation is on, only the Gaussian .out file for TS2A and TS2B are required')
            print('')
            quit()

    if args.intermediate_option == True:
        if args.GausFile7_option == None or args.GausFile8_option == None:
            print('')
            print('ERROR')
            print('Please provide TS2A and TS2B Gaussian 16 frequency calculation output files when intermediate activation is on')
            print('')
            quit()

    if args.intermediate_option == False:
        if args.GausFile7_option != None or args.GausFile8_option != None:
            print('')
            print('ERROR')
            print('TS2A and TS2B Gaussian 16 frequency calculation output files are not required when intermediate activation is off')
            print('')
            quit()


    if args.noTS1_option == False:
        main1(args.GausFile1, args.GausFile2, args.GausFile3, args.GausFile4, args.GausFile5, args.GausFile6, args.GausFile7_option, args.GausFile8_option, args.intermediate_option, args.weight_option)

    if args.noTS1_option == True:
        main2(args.GausFile1, args.GausFile2)
